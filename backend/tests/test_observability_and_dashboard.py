...
def test_request_logging_includes_correlation_metadata_when_otel_enabled(
...
def test_internal_dashboard_allowlist_and_overview(temp_data_dir, monkeypatch):
    ...
    monkeypatch.setattr(
        settings, "internal_test_password", "correct horse battery staple"
    )
    monkeypatch.setattr(
        settings,
    ...
    monkeypatch.setattr(
        settings, "internal_dashboard_allowlist_raw", "admin@example.com"
    )
...
def test_analytics_capture_requires_auth_for_non_public_events(
    temp_data_dir, monkeypatch
):
    from main import create_app
    ...
    monkeypatch.setattr(
        settings, "internal_test_email_allowlist_raw", "admin@example.com"
    )
        ...
        return await run_agent(
            {
                ...
                "await_search_tool_request": lambda request_id, timeout_s: (
                    asyncio.sleep(0)
                ),
            },
            [],
