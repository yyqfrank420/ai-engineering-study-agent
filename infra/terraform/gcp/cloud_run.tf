...
resource "google_cloud_run_v2_service" "backend_staging" {
  ...
  template {
    ...
    containers {
      ...
      dynamic "env" {
        for_each = local.staging_env_vars
        content {
          name  = env.key
          value = env.value
        }
  ...
  depends_on = [
    google_artifact_registry_repository.backend,
    google_project_iam_member.staging_artifact_registry_reader,
    google_secret_manager_secret_iam_member.staging_secret_accessor,
  ]
}
